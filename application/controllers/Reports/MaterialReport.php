...
class MaterialReport extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if (isset($search["clearall"])) {
            ...
            $this->session->set_userdata('materialreport', $session_data);
            ...
            if (!empty($this->session->userdata['materialreport']['sdate']) && $this->session->userdata['materialreport']['sdate'] != "") {
                $sdate = $this->session->userdata['materialreport']['sdate'];
                $date = new DateTime($sdate);
                $searchtxt["sdate"] = $date->format('Y-m-d');
                $sdate = $this->session->userdata['materialreport']['sdate'];
            }
            if (!empty($this->session->userdata['materialreport']['edate']) && $this->session->userdata['materialreport']['edate'] != "") {
                $edate = $this->session->userdata['materialreport']['edate'];
                $date = new DateTime($edate);
                $searchtxt["edate"] = $date->format('Y-m-d');
                $edate = $this->session->userdata['materialreport']['edate'];
            }
            if (!empty($this->session->userdata['materialreport']['itemno'])) {
                $itemno = $this->session->userdata['materialreport']['itemno'];
                $searchtxt['itemno'] = $itemno;
            }
            if (!empty($this->session->userdata['materialreport']['customerid'])) {
                $customerid = $this->session->userdata['materialreport']['customerid'];
                $searchtxt['customerid'] = $customerid;
            }
            if (!empty($this->session->userdata['materialreport']['sortbycol'])) {
                $sortbycol = $this->session->userdata['materialreport']['sortbycol'];
                $searchtxt['sortbycol'] = $sortbycol;
            }
            if (!empty($this->session->userdata['materialreport']['sortby'])) {
                $sortby = $this->session->userdata['materialreport']['sortby'];
                $searchtxt['sortby'] = $sortby;
            }
        }
        ...
        $total_records = $this->QueriesReport->getMaterialReportCount($searchtxt);
        $params['materialreport'] = $this->QueriesReport->getMaterialReport($searchtxt, $limit_per_page, $start_index);
        ...
        if ($total_records > 0) {
            ...
            $config = PageConfig(base_url() . 'Reports/MaterialReport/index', $total_records, $limit_per_page, '4');
        ...
        $query = "select id,item_no,part_type from " . TBL_ORDER . " where isdelete=0 group by item_no";
        $params["itemlist"] = $this->Queries->get_tab_list_two($query, 'item_no', 'item_no', 'part_type');


        $this->load->view('Reports/MaterialReport/index', $params);
    }
    ...
    //         $date = new DateTime($row->start_preparing);
    //         $sdate = $date->format('d-m-Y H:m A');
    //         $date = new DateTime($row->end_removing);
    //         $edate = $date->format('d-m-Y H:m A');
    //         $date = new DateTime($row->attach_furnace);
    //         $fdate = $date->format('d-m-Y H:m A');
